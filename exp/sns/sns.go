...
func (sns *SNS) ListTopics(NextToken *string) (resp *ListTopicsResp, err error) {
	...
	err = sns.query("GET", nil, nil, params, resp)
	return
}
...
func (sns *SNS) CreateTopic(Name string) (resp *CreateTopicResp, err error) {
	...
	params["Name"] = Name
	err = sns.query("POST", nil, nil, params, resp)
	return
}
...
func (sns *SNS) query(method string, topic *Topic, message *Message, params map[string]string, resp interface{}) error {
	params["Timestamp"] = time.Now().UTC().Format(time.RFC3339)
	url, err := url.Parse(sns.Region.SNSEndpoint)
	if err != nil {
		return err
	}
	sign(sns.Auth, method, "/", params, url.Host)

	var request *http.Request
	if method == "POST" {
		body := bytes.NewBufferString(multimap(params).Encode())
		request, _ = http.NewRequest("POST", url.String(), body)
